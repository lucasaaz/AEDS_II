...
class Lista {
    ...
    public void sort() {
        ...
        ClasseFilme[] tmp = new ClasseFilme[n+1];
        for(int i = 0; i < n; i++){
           tmp[i+1] = array[i];
        }
        array = tmp;
    ...
    private void quicksort(int esq, int dir) {
        int i = esq, j = dir;
        ClasseFilme pivo = array[(dir + esq) / 2];
        while (i <= j) {
            while (array[i].getsituacao().compareTo(pivo.getsituacao()) < pivo.getsituacao().compareTo(array[i].getsituacao())) {
                i++;
            }
            while (array[j].getsituacao().compareTo(pivo.getsituacao()) > pivo.getsituacao().compareTo(array[j].getsituacao())) {
                j--;
            }
            if (i <= j) {
                swap(i, j);
    ...
    public void insertsort() {
        for (int i = 1; i < n; i++) {
            ClasseFilme tmp = array[i];
            int j = i - 1;

            while ((j >= 0) && (array[j].getnome().compareTo(tmp.getnome()) > tmp.getnome().compareTo(array[j].getnome()))) {
                array[j + 1] = array[j];
                j--;
                contador++;
            }
