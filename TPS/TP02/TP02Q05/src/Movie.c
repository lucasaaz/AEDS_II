...
int main() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        scanf(" %[^\n]", auxL); //ler a linha
        strcpy(auxN, substring(auxL, 4, strlen(auxL))); //subs pra retornar sem I*
        
        if(auxL[0] == 'I' && auxL[1] == 'I'){ //compara se tem isso ba linha

            movie_start(&movie); //começa 
            movie_readHtml(&movie, auxN); //le a o arquivo
            inserirInicio(movie); //coloca no incio 
        }else if(auxL[0] == 'I' && auxL[1] == 'F') {

            movie_start(&movie);
            movie_readHtml(&movie, auxN);
            inserirFim(movie);
        }else if(auxL[0] == 'I' && auxL[1] == '*') {

            strcpy(pos, substring(auxL, 4, 2));
            //printf("Pos: %s\n", pos);
            strcpy(namePos, substring(auxL, 7, strlen(auxL)));
            //printf("NamePos: %s\n", namePos);
            movie_start(&movie);
            movie_readHtml(&movie, namePos);
            inserir(movie, atoi(pos));
        }else if(auxL[0] == 'R' && auxL[1] == 'I') {

            movie = removerInicio();
            printf("(R) %s\n", movie.name);
        }else if(auxL[0] == 'R' && auxL[1] == 'F') {

            movie = removerFim();
            printf("(R) %s\n", movie.name);
        }else if(auxL[0] == 'R' && auxL[1] == '*') {

            strcpy(pos, substring(auxL, 4, 2));
            movie = remover(atoi(auxN));
            printf("(R) %s\n", movie.name);
        }
    }
